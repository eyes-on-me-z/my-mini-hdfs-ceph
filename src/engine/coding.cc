...
namespace mini_storage
{
    ...
    {
        // 按小端序读取 buf[0..3]，把 4 个字节拼成一个 32 位无符号整数
        // 使用的是 小端序 解码
        // char 可能是有符号的。如果某个字节大于 0x7f，直接转可能出现符号扩展问题。
        // 先转成 unsigned char，就能保证它只是一个普通的 0~255 字节值。
    ...
    {
        // 每个字节只存 7 位有效数据，最高位用来表示后面还有没有字节
        // 1：后面还有字节。0：这是最后一个字节。
        int len = 0;
        while(value >= 128)
        {
    ...
    {
        uint32_t len;
        int consumed = DecodeVarint32(*buf, &len);   // 长度字段本身占了几个字节
    ...
    {
        // ((crc >> 24) & 0xff) ^ p[i]
        // 拿当前 CRC 的最高 8 位和当前输入字节做异或，得到一个 0~255 的索引
        uint32_t crc = icrc;
